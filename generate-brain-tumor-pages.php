<?php
// Generate all state and city pages for brain-tumor condition
...
foreach ($states as $state => $cities) {
    ...
    $stateDir = __DIR__ . '/brain-tumor/' . $stateSlug;
    if (!is_dir($stateDir)) mkdir($stateDir, 0755, true);

    file_put_contents($stateDir . '/index.php', "<?php\n\$state = '$state';\n\$area = '$state';\ninclude __DIR__ . '/../../includes/master-brain-tumor-template.php';\n?>");
    ...
    echo "brain-tumor: $state (" . count($cities) . " cities)\n";
}

file_put_contents(__DIR__ . '/brain-tumor/index.php', "<?php\n\$state = 'India';\n\$area = 'India';\ninclude __DIR__ . '/../includes/master-brain-tumor-template.php';\n?>");
echo "brain-tumor pages generated!\n";
?>
<?php
/**
 * Comprehensive page generator for all conditions
 * Creates all state and city pages for: blood-clot, brain-aneurysm, brain-tumor, headache, hydrocephalus
 */

$conditions = ['blood-clot', 'brain-aneurysm', 'brain-tumor', 'headache', 'hydrocephalus'];

// All states with their cities from back-pain-treatment structure
$allStates = [
...
foreach ($conditions as $condition) {
    ...
    foreach ($allStates as $stateName => $cities) {
        ...
        // Fix state slugs with special characters
        ...
        foreach ($cities as $city) {
            ...
            // Fix city names with special characters
            $cityName = str_replace('&', 'and', $cityName);
        ...
        echo "  - $stateName: " . count($cities) . " cities\n";
    }

    echo "  Total: $totalStates states, $totalCities cities\n\n";
}

echo "All pages generated successfully!\n";
echo "\nFiles created per condition:\n";
foreach ($conditions as $condition) {
    $count = 0;
    $dir = __DIR__ . '/' . $condition;
    $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($dir));
    foreach ($iterator as $file) {
        if ($file->getFilename() === 'index.php') $count++;
    }
    echo "  $condition: $count pages\n";
}
?>